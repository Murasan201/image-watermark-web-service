...
**詳細ログ（3ファイル、中央配置、フォントサイズ270での例）**:
```
SVG generation for 113.jpg: {
  imageSize: '1280x670',
  fontSize: 270,
  textWidth: 2592,
  calculatedPosition: '(640, 335)',  // 正しい中央座標が計算されている
...
3. **座標計算とレンダリングの不整合**:
   - 計算上は正しい座標だが、実際のレンダリング時にずれる
   - フォント依存による文字サイズ計算の誤差

**実施済み対策**:
- ✅ フォント指定をDejaVu Sansに変更
- ✅ SVG座標計算の境界チェック無効化
- ✅ 一時的にクライアント処理（CLIENT_MULTI）に変更するも、まだサーバー処理が実行される
- ✅ 詳細なデバッグログ追加
- ✅ SVG→PNG変換アプローチの試行

**検討中の対策**:
1. **完全なクライアント処理移行**:
   - 複数ファイルもCanvas APIで順次処理
   - サーバーサイド処理の完全回避

2. **代替フォント処理**:
   - Web Fontの使用
   - Canvas APIでのテキスト描画に統一
...
**現在の暫定対応**:
- 複数ファイル処理時はクライアントサイド処理を推奨
- サーバーサイド処理は1ファイルのみに制限

**再現条件**:
- ファイル数: 2ファイル以上
- 処理方式: サーバーサイド（Sharp）
- ウォーターマーク設定: 任意（位置・フォントサイズ問わず）
- 環境: Vercel本番環境

**調査継続ポイント**:
- Fontconfig設定の代替方法
- SVGテキストレンダリングの環境依存性
- Sharp処理での座標計算とレンダリングの整合性確認
